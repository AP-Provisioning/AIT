...
function Step-oobeUpdateWindows {
    ...
    if (($env:UserName -ne 'defaultuser0') -and ($Global:oobeCloud.oobeUpdateWindows -eq $true)) {
        ...
        if (Get-Module PSWindowsUpdate -ListAvailable -ErrorAction Ignore) {
            ...
            Start-Process PowerShell.exe -ArgumentList "-Command Install-WindowsUpdate -MicrosoftUpdate -AcceptAll -IgnoreReboot -NotTitle 'Preview' -NotKBArticleID 'KB890830','KB5005463','KB4481252','KB5068861','KB5072033','KB5066131'" -Wait
        }
    }
}
